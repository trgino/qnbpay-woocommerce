...
Ödeme API'si, sipariş ve kredi kartı ayrıntı bilgilerini QNBpay ödeme entegrasyon sistemine göndermek için kullanılır. Üye işyeri web sitesi, işlem sonrası ödeme durumunu /api/checkstatus ile doğrulamalıdır. API başarı durumuna göre, alışveriş sepeti ve sipariş durumu buna göre değiştirilmelidir. Bu ödeme API'sinde diğer ödeme apilerde gibi getpos API 'yi çağırmaya gerek yoktur.
...
hash\_key isteğin QNBpay'den geldiğini doğrulamak için üye iş yeri tarafından kullanılır Ayrıca İşlem Durumu servisi işlemin başarılı veya başarısız olduğunu doğrular.
...
cancel\_url, ödeme başarısız olduğunda yönlendirme için kullanılır ve ödeme başarılı olduğunda yönlendirme için return\_url kullanılır.
...
qnbpay\_status, order\_id ve invoice\_id. Örneğin, başarılı URL https://alanadim.com/ ise, başarılı ödemeden sonra QNBPAY, https://alanım.com/?qnbpay\_status=1&order\_no=234234232&invoice\_id=73434 adresine yönlendirecektir.

Sorgu dizesinde, qnbpay\_status = 1 ise, sepeti temizlemek ve sipariş durumunu "Tamamlandı" olarak değiştirmek için "/api/checkstatus" çağırılmalıdır.
...
**Sadece Sigorta Üye İşyerlerinin Ödemeleri İçin;**

vpos\_type : Yalnızca Sigorta Üye işyeri tarafından bir sigorta ödemesi başlatmak için geçerlidir. Sadece PaySmart2d servisinde kullanılır, Mevcut değer "insurance" olmalıdır, aksi takdirde hata alınır.

identity\_number : Yalnızca "vpos\_type":"insurance" olduğunda gönderilmelidir, İlgili Değer (tckn/vkn/tin) 10-11 basamak arasında olmalıdır.

cc\_no : "ilk\_sekiz\_hane + \*\*\*\*\*\* + son\_dört\_hane" biçiminde gönderilmelidir, Örnek olarak cc\_no 1234567897891234 ise cc\_no "12345678\*\*\*\*1234" olarak gönderilmelidir ( lütfen yıldızların sayısını dikkate alın , her zaman dört (4) yıldız olarak gönderilmelidir, aksi takdirde hata alınır. )

**Vade Farkını Kart Sahibi Ödesin**

Vade farkının kart hamili tarafından ödenmesi istenen durumlarda ödeme isteğine "is\_comission\_from\_user" : 1 ve "is\_comission\_from\_user" : 1 parametreleri ve değerleri eklenmelidir.

**Post**(/api/paySmart2D)

| Test | https://test.qnbpay.com.tr/ccpayment/api/paySmart2D |
| --- | --- |
| Canlı | https://portal.qnbpay.com.tr/ccpayment/api/paySmart2D |
...
```
{
      "cc_holder_name":"Aydın Öztürk",
      "cc_no":"4022780520669303",
      "expiry_month":"01",
      "expiry_year":"2050",
      "cvv":"988",
      "currency_code":"TRY", 
